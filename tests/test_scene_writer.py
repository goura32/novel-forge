...
class TestAssembleChapter:
    ...
    def test_assemble_creates_chapter_file(self, tmp_path):
        writer = self._make_writer(tmp_path)
        chapter = type("Ch", (), {"number": 1, "title": "第1章"})()
        scene_texts = ["シーン1（第1章）: 本文1", "シーン2（第1章）: 本文2"]
        result = writer.assemble_chapter(1, chapter, scene_texts, [1, 2])
        assert Path(result).exists()
        content = Path(result).read_text(encoding="utf-8")
        assert "# 第1章" in content
        assert "本文1" in content
        assert "本文2" in content

    def test_assemble_strips_scene_markers(self, tmp_path):
        writer = self._make_writer(tmp_path)
        chapter = type("Ch", (), {"number": 1, "title": "第1章"})()
        scene_texts = ["シーン1（第1章）: 本文1"]
        result = writer.assemble_chapter(1, chapter, scene_texts, [1])
        content = Path(result).read_text(encoding="utf-8")
        assert "シーン1" not in content
        assert "本文1" in content

    def test_assemble_saves_version0_files(self, tmp_path):
        """assemble_chapter should save version=0 (no suffix) files."""
        writer = self._make_writer(tmp_path)
        chapter = type("Ch", (), {"number": 1, "title": "第1章"})()
        # Pre-create v1 files (simulating write_scene output)
        ch_dir = writer._series_dir / "vol01" / "vol01_ch01"
        ch_dir.mkdir(parents=True)
        (ch_dir / "vol01_ch01_sc01_v1.md").write_text("本文1", encoding="utf-8")
        (ch_dir / "vol01_ch01_sc02_v1.md").write_text("本文2", encoding="utf-8")
        scene_texts = ["本文1", "本文2"]
        writer.assemble_chapter(1, chapter, scene_texts, [1, 2])
        assert (ch_dir / "vol01_ch01_sc01.md").exists()
        assert (ch_dir / "vol01_ch01_sc02.md").exists()

    def test_assemble_version0_readable_by_load(self, tmp_path):
        """version=0 files created by assemble_chapter should be readable by load_scene_draft."""
        writer = self._make_writer(tmp_path)
        chapter = type("Ch", (), {"number": 1, "title": "第1章"})()
        ch_dir = writer._series_dir / "vol01" / "vol01_ch01"
        ch_dir.mkdir(parents=True)
        (ch_dir / "vol01_ch01_sc01_v1.md").write_text("本文1", encoding="utf-8")
        scene_texts = ["本文1"]
        writer.assemble_chapter(1, chapter, scene_texts, [1])
        result = writer.load_scene_draft(1, 1, 1)
        assert result == "本文1"
