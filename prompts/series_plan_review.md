# シリーズ企画の自己レビュー

## 指示

以下のシリーズ企画テキストを評価し、改善点を指摘せよ。

## 入力

- シリーズ企画:
```
{series_plan}
```
- 出力言語: `{lang}`

## 評価基準

1. **言語純度**: {lang}の文字のみを含み、他言語(中国語・英語等)の混在がないか
   - **除外するもの**: 日本語で標準的に使われる記号（・、。、「」『』（）！？…ー等）は言語制約違反としない
   - **除外するもの**: 日本語の漢字・ひらがな・カタカナは言語制約違反としない
   - **検出対象**: 英語の単語（the, and, is 等）、簡体字（转、间、门等）、ハングル（가、나、다等）
   - **例外（技術用語 — 英語のまま許可）**: CPU, GPU, SSD, USB, URL, HTML, RAM, ROM, DNS, VPN, SSH, FTP, API, SDK, LAN, WAN, GPS, LED, LCD, OLED, DNA, RNA, CT, MRI, X線, IoT, PC, AI, ICU
   - **例外（和製英語・定着語 — 日本語として許可）**: データ、ファイル、名前、兵器、扉、走る、考える、見る、聞く、光、壁、画面、声、手、心、顔、目等
2. **市場競争力**: 競合との差別化が明確か
3. **ターゲット適合**: 想定読者が具体的か
4. **シリーズ持続性**: 複数巻にわたる展開が見込めるか
5. **タイトル力**: 検索・宣伝に有効か
6. **キャッチコピー**: 読者の興味を引くか

**評価対象について:** 入力テキストに含まれるすべての文字列を評価対象とすること。JSONキー名は含まれていない。

**slug の例外:** `slug` フィールドはローマ字のハイフン区切りパスが許可される。slug のローマ字を言語制約違反として指摘してはならない。

**医療用語の例外:** `ICU`, `DNA`, `RNA` 等の医療・科学分野の英語略語は、日本語として定着しているものに限り許可される。

`score` の計算方法は `series_plan_review.json` の `score` の description に従うこと。

## スコア制約

`score` は **0 以上 100 以下** の整数とすること。
100点満点や1000点満点など、異なる尺度で出力してはならない。

**スコア計算ルール（厳格適用）:**
- 値に英語混在がある場合: 混在1件につき **-10**（最低 0）
- 値に簡体字・ハングルが含まれる場合: **最大 30**
- 言語純度100%で企画内容が優秀: 80〜90
- 言語純度100%で企画内容が平凡: 60〜70
- 言語純度100%だが企画に致命的欠陥: 50〜60

## 出力スキーマ

`series_plan_review.json` に適合する JSON を出力すること。

**以下のJSONテンプレートの構造とフィールド名を厳守すること。フィールド名や構造を変更しないこと。**

```json
{
  "score": 85,
  "issues": [
    {
      "severity": "critical",
      "category": "カテゴリ名",
      "description": "問題の説明",
      "suggestion": ["改善提案1", "改善提案2"]
    }
  ],
  "strengths": ["強み1", "強み2"],
  "recommendations": ["推奨事項1", "推奨事項2"]
}
```

**注意**:
- 上記テンプレートのキー名（score, issues, strengths, recommendations）は変更しないこと。
- `issues[].severity` は「critical」「major」「minor」から選択すること。
- `issues[].suggestion` は**文字列の配列**であること。各要素は1つの改善提案。
- **JSONの文字列値内に改行（`\n`）を含めないこと。長い文章は1行で記述すること。改行が必要な場合は `\n` と記述すること。**
