...
- `scene_closure`: シーン末尾の自然さ。シーンが唐突に切れていないか。読者がシーンの終わりを自然に感じられるか。**シーンの位置（中間シーン等）では、自然な区切りで終わることも許容する。** すべてのシーンに劇的なクロージングを求めるのは不自然。**減点するのは、シーンが唐突に切れたり、意味不明な箇所で終わっている場合のみ。**
