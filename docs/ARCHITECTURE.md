...
| フェーズ | 時間 | 備考 |
|---|---|---|
| アンロード (`keep_alive:0`) | 0.9s | 即座にメモリから解放 |
| **モデルロード** | **13.0s** | ディスク → GPU メモリ転送 |
| プロンプト評価 | 0.3s | |
| 推論（1トークン） | 0.2s | |
| **コールドスタート合計** | **13.4s** | |
| warm（メモリ済み） | 0.5s | ロード不要 |

### 6.3 タイムアウト設計

各工程のタイムアウト値を実測データに基づいて設定。

| 工程 | 期待される応答 | タイムアウト | 根拠 |
|---|---|---|---|
| プリロード（keep_alive） | 13s（コールド）/ 0.5s（warm） | 30s | コールドスタート 13s の2倍を余裕 |
| 短文生成（100字） | 0.5〜2s | 15s | warm 時 0.5s の30倍 |
| 中段生成（200字） | 1〜3s | 30s | warm 時 1.5s の20倍 |
| 長文生成（1000字） | 2〜8s | 60s | warm 時 1s の60倍 |
| 超長文（企画・構成） | 5〜30s | 120s | num_tokens=4096+ |

プリロードのタイムアウトはコールドスタートを想定。以降の生成は warm を想定し、タイムアウトに十分な余裕を持たせる。
