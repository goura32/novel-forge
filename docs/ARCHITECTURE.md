...
|---|---|---|---|
| プリロード（keep_alive） | 2〜10s | 30s | コールドスタートを想定 |
| 短文生成（100字） | 0.5〜2s | 15s | warm 時 0.5s の10倍を余裕 |
| 中段生成（200字） | 1〜3s | 30s | warm 時 1.5s |
| 長文生成（1000字） | 2〜8s | 60s | warm 時 1s 実測（num_tokens=2048） |
| 超長文（企画・構成） | 5〜30s | 120s | num_tokens=4096+ |

タイムアウトは `timeout_errors` としてキャッチし、リトライする。
