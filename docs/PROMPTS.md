...
LLM の応答に簡体字（中国語簡体字）が混じった場合、品質ゲートで `critical` として検出する。

**検出手法**: 変換ライブラリ（opencc-purepy）を使用。簡体字を繁体字に変換できる文字を特定する。変換された文字が1文字でもあれば、そのシーンは簡体字混入ありと判定する。

**リトライ**: 行わない。検出結果を品質レコードに記録し、`kdp_readiness_report.md` に集約する。人間が確認・判断する。

**防止策**: システムプロンプト（`system.md`）に「简体中文・簡体字は使用しない。必ず日本語で出力する」と明記する。
