...
各プロンプトの先頭付近に `## 応答方針` セクションを記述し、LLMに期待する役割・判断基準・品質基準を明示する。
...
**レビュー可能な工程に「言語純度」カテゴリを導入**

- 日本語文脈で不自然な簡体字、中国語構文、英語混在、ハングル混在を禁止する
- 自然なカタカナ語、英語表記、英字略語、一般的なジャンル語、固有名詞、日本語として成立する漢語は問題にしない
- 指摘時は問題文字列を引用し、日本語文脈で不自然な理由を短く示す
- 表紙画像生成など、スキーマ上で英語プロンプトを要求する指定フィールドは例外とする
...
- **言語純度**: 日本語文脈で不自然な簡体字、中国語構文、英語混在、ハングル混在がないか。自然なカタカナ語、英語表記、英字略語、一般的なジャンル語、固有名詞、日本語として成立する漢語を誤指摘していないか
