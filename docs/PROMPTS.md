...
## 必須フィールド判定（Plan フェーズ、2026-08-07 更新）

Plan フェーズのレビューでは、フィールドを「主要」と「補足」に分けて判定します。

| 分類 | フィールド | 欠落時の深刻度 |
|---|---|---|
| 主要 | name, role, personality, motivation, flaw, growth | **重大** |
| 補足 | gender, age, occupation, appearance, background, arc | **重要** |

補足フィールドはレビューで修正を促しつつ、即座に「重大」で再生成しません。

**Design/Write フェーズ**では、すべてのフィールドが `required` なしで定義され、レビューで品質を確保します。
