...
| `{context}` | シリーズ企画 + 巻アウトライン + 事実記録（Blackboard）facts + 設定資料集（Bible）情報 | context 注入 |
...
3. **感覚描写**: 7感覚（視覚・聴覚・嗅覚・触覚・味覚・固有感覚・内臓感覚）を意識し、シーンに最低3つ以上の感覚描写を含める。全7つの網羅は必須ではなく、シーンに適った感覚を選択すること
...
`scene_quality_gate.md` の評価カテゴリに以下を追加する。詳細は [GLOSSARY §6](GLOSSARY.md) を参照。
