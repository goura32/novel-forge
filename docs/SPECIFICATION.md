...
- `chapter_number`: 章番号
- `theme`: 章のテーマ
- `scene_summaries`: 全シーンの要約
- `emotional_arc`: 章の感情アーク
- `act_role`: 幕の役割（`setup` / `confrontation` / `resolution`）。三幕構成のどの幕に該当するか
...
- `state.json` のステータスが `finalized`、`exported`、または `force_exported` に更新されていること
...
**シーン単位の文字数フィードバック**: 各シーンの Draft 生成後、文字数を計測し上限（4,000字）を超過している場合はプロンプトに警告を注入して改稿を促す。Quality Gate でも文字数超過を `major` issue として検出する。

### 7.4 言語制約

**中国語の生成を禁止する。** 出力言語は `--lang` で指定された言語に限定する。日本語指定の場合、中国語の漢字・語句を一切含めない。中国語が混在した場合は生成を拒否し、再生成を要求する。詳細は [PROMPTS.md §78–82](PROMPTS.md) を参照。

### 7.5 長文への対応

1. **分割処理**: 各工程は scene または chapter 単位で LLM に投入
2. **集約の階層化**: scene → chapter → volume の順に段階的に集約
3. **事実記録の巻ごとの要約**: 前巻の全データではなく、要約のみを次の巻に引き継ぐ。詳細は [ARCHITECTURE §8](ARCHITECTURE.md) を参照
