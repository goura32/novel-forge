...
| P18-19 | 実LLM smoke を5回目実行 | In progress | `proc_134341984a7b` / `workspace/phase18_real_smoke_20260706_114505` で同条件実行中。Designを抜けてWriteへ進むか確認 |
| P18-20 | `system.md` を別タスクでレビュー | Todo | 実LLM smoke 後。JSON only 指示、役割混同、品質方針との矛盾を確認 |

### Phase 18 復帰メモ

- 現在の正: この `PROGRESS.md`。中断復帰時は P18-19 以降から再開する。
- 直近検証済みコマンド:
  - `uv run pytest` → 292 passed
  - `git diff --check` → OK
  - `uv run ruff check src tests` → All checks passed
- 次に迷わず実行すること:
  1. `process(action="poll", session_id="proc_134341984a7b")` で smoke の終了を確認
  2. `workspace/phase18_real_smoke_20260706_114505/novel_forge.log` と `_raw_logs` を読む
