...
| P18-26 | 実LLM smoke を8回目実行 | Todo | P18-25修正後に `--max-generation-count 3 --max-review-count 4 --verbose` で再実行。Design通過とWrite到達を確認 |
...
- 次に迷わず実行すること:
  1. P18-25を commit/push
  2. P18-26として `--max-review-count 4` で実LLM smoke を再実行
