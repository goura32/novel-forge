...
- Relationship Arc は Core ↔ Core を原則作成し、Core / Supporting ↔ Minor は再登場、継続する秘密・負債・約束、または Design Intent がある場合だけ作る。Local / Collective との Arc は作らない。
...
| location | `loc_*` | character current location / artifact location / Context scope | 場所名の部分一致で同一視する |
...
| artifact | holder / location / condition の明示 update |
| knowledge | holder state / visibility の明示 update |
...
| `artifact` | 固有の能力・制約・所有/所在・状態を持つ重要物 | 汎用品、単発の小道具 | 所有・能力・損壊・所在が後続に影響する時 |
...
```jsonc
{
  ...
  "artifacts": [{
    ...
    "holder": {"id": "char_001"}, "location": {"id": "loc_stone_city"},
...
- `Chronology` は singleton である。時刻・期限が物語上意味を持つ場合だけ seed / scene patch で更新し、scene ごとの重複要約や技術的 event order を保存しない。
...
- 必須 world rule / series constraint / POV の knowledge / active deadline は token budget によって脱落させない。
...
| artifact | Plan Seed または scene patch create | holder、location、condition | property の訂正は correction | scope 内の design / review / writer projection |
...
| knowledge | Plan Seed または scene patch create | holder knowledge state、visibility、reveal | proposition / truth status の訂正は correction | author-context design / review、POV-filtered writer projection |
...
```jsonc
{
  "canon_patch": {
    ...
    "artifacts": {"create": [], "state_updates": []},
...
- Location state update は setting または scene outcome に根拠を要する。Artifact update は holder / location / condition のうち実際に変化した field だけを持ち、holder と location の参照整合性を検証する。
- Knowledge holder update は、holder が scene cast であるか、明示的な伝達・観察・推論の scene outcome を根拠に持つ。author truth を holder の `knows` へ自動昇格してはならない。
- Chronology は `advance_to` の単調前進と deadline の `active → resolved / missed` だけを通常 patch で許可する。過去 marker への巻戻しは source replacement または correction に限る。
...
- `world_rules`、series constraint、Location の immutable constraint、Artifact の properties、Knowledge の proposition / truth status、既存 profile 訂正は patch に含めない。
...
|---|---|---|
| **P0: invariant** | series constraint、scope に関係する world rule、POV / cast の identity と current state、setting の制約、active deadline、POV knowledge | token budget で除外しない |
...
- `current_state`、`condition`、`holder`、`location`、期限、relationship perspective は要約で落としてはならない。長文の `arc_summary`、background、過去履歴だけを compact 化できる。
...
write / export から Bible / Canon Event の load / save を行う経路を残さない。writer の input は `writer_context` と直近 scene summary のみであり、旧 prompt placeholder を経由した全 Canon 注入も残さない。
...
- Character identity、promotion、identity reveal、2 participant / 2 perspective、affiliation、cast relevancy、knowledge/POV 分離、artifact holder/location、deadline transition の RED test を先に追加する。
...
- Minor create、role-anchored identity reveal、Minor → Supporting promotion、Collective affiliation、Location 封鎖、Artifact 移送 / 損壊、Knowledge reveal / POV leak 拒否、Chronology deadline、Relationship Arc の非対称 perspective update を検証する。
...
- [ ] Location は不変制約と現在状態を、Artifact は不変 properties と holder / location / condition を分離し、通常 patch が不変 field を書換えない。
- [ ] Knowledge は authorial proposition / truth status と holder ごとの認識状態を分離し、holder を author truth から自動的に `knows` へ昇格しない。
- [ ] Foreshadowing は setup/payoff、Knowledge は知識分布、Subplot は `dramatic_question` / `stakes` / `current_state` を持つ独立 Arc として責務分離される。
- [ ] Chronology は物語時刻と active deadline だけを保持し、通常 patch の時刻は単調前進する。
