...
```jsonc
{
  ...
  "characters": [{
    ...
    "continuity_card": {"current_state": "石の都へ到着した", "distinguishing_traits": "石粉の残る指先", "known_constraints": []},
    "affiliations": [],
    ...
    "profile": null,
    "continuity_card": {"current_state": "主人公の通行証を疑っている", "distinguishing_traits": "左手の青い染料痕", "known_constraints": ["王都の監察官に借りがある"]},
...
- `importance` は物語上の重み、`tracking_level` は Canon に保持する情報量であり、同義ではない。
...
| collective | `grp_*` | affiliation / collective stance | 個人の Relationship Arc participant にする |
...
| character | `continuity_card.current_state` 更新、明示 `minor → supporting → core` 昇格 |
  ...
  "canon_patch": {
    "characters": {
      "create": [{
        ...
        "profile": null,
        "continuity_card": {"current_state": "主人公を監視している", "distinguishing_traits": "右頬の古傷", "known_constraints": []},
...
- 新規 character / collective / relationship / subplot / glossary / foreshadowing の create payload は、各 entity の必須プロパティを全て持つ。
...
- design は scene cast に含まれる Core / Supporting の relevant profile、Minor の `continuity_card`、該当する Relationship Arc、Collective state、親 Intent だけを Bible slice として読む。
- 無関係な人物、関係 Arc、Collective を scene prompt に注入しない。
- write は Bible / Canon Event を読まない。design が参照した関係制約は `scene_design.relationship_context` として scene artifact に投影し、writer はその投影だけを読む。
...
- `SceneId`、`SourceRef`、typed reference、Character Tier、Collective、Relationship Arc、status transition、review evidence validator を実装する。
- Character identity、promotion、identity reveal、2 participant / 2 perspective、affiliation、cast relevancy の RED test を先に追加する。
- source replacement、dependency rejection、replay の RED test を先に追加する。

### Phase 2 — Storage / seed / Event Store

- `BibleFactory` が plan 完了時に `bible_seed.json` を作る。Core、初期 Supporting、初期 Collective、初期 Relationship Arc は plan seed に含める。
- Minor は必要になった review 合格済み scene patch でだけ create する。Local role は seed / Canon のどちらにも保存しない。
...
- volume / chapter schema に構造化 `design_intent`、relationship arc intent、cast intent を追加する。
- scene schema にだけ `cast`、`relationship_context`、`canon_patch` を追加する。
- scene review に cast-relevant Bible slice、親 Intent、Character Tier / Relationship Arc patch validator、review evidence を接続する。
...
- writer が `scene_design.relationship_context` だけを使い、Bible の全人物 / 関係性を load しないことを regression test で固定する。

### Phase 5 — E2E

- fake LLM で plan → volume/chapter intent → scene patch → write → export を検証する。
- Minor create、role-anchored identity reveal、Minor → Supporting promotion、Collective affiliation、Relationship Arc の非対称 perspective update を検証する。
...
- [ ] scene design は cast-relevant slice だけを読み、writer は投影済み `relationship_context` だけを読む。
