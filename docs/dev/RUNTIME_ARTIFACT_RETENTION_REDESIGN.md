...
> **Note:** `plan` は単一の `plan.series.generate` に収束した（旧 `plan.concept/characters/volumes` の3分割は廃止）。plan は review/revise チェーンを持たない。`design.chapter` 系は registry に予約されているが、現在の `RuntimeWorkflow` は `design.volume` のみを使用する。
