...
keywords
  ...
  → selection snapshot
  → design.volume / design.chapter / design.scene.generate
...
`plan` は単一の `plan.series.generate` taskです。`design.volume` / `design.chapter` / `design.scene` はgenerate-onlyで、scene designはdeterministic reviewとCanon event公開を通ります。`write.draft` と `write.summary` はgenerate / review / reviseを持ちます。
...
scene designはCanon patchを要求され、runtimeが名前ベースのauthor contextをtyped referenceへ解決します。空またはno-opのpatch、存在しない参照、frontier replayの不整合は拒否されます。writerへ渡すのはwriter-safe contextと直近summaryであり、raw Canon frontierやstable IDを直接渡しません。
