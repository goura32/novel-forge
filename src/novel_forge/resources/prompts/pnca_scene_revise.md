...
- `coverage.evidence` は本文ではない。各 required beat と end constraint について、完了を直接示す `content` 内の完全一致文を一つずつ引用する。要件が完了していないなら引用を捏造せず、先に本文を完成させる。引用文を書き直す・言い換える・句読点や空白を変えると、本文中の該当文と完全一致しなくなり publication gate で拒否される。改訂で文を調整する場合は、coverage の `draft_quote` と本文内の該当文が常に一致するよう、どちらかを崩さず同期させる。
...
- 出力は JSON オブジェクト**のみ**。スキーマ定義、キー説明、`"type"`・`"properties"` 等のスキーマ構造を output に含めてはならない。
- `content` は改訂後のシーン本文そのものを**文字列**として入れる（例: `{"content": "本文…"}`）。`content` にオブジェクトやスキーマを入れてはならない。
- それ以外のフィールド（coverage 等）は、入力で与えられた構造をそのまま維持する。
