...
- `start_context` に「浮遊する石」「魔力の暴走」「立てない状態」とあるなら、scene はそのままその状態から開始し、前段の出来事を回想・再現・言い換えで書き直してはならない。## 入力情報
### start_context
{start_context}
...
出力前に、POV人物以外を主語にした「知っている」「思う」「望む」「感じる」「気づく」「戦っている」「～ように見えた」のような内面・意図の断定が本文にないか確認し、あれば台詞・動作・音・外見、またはPOV人物の推測（「私には～に見えた」「～としか思えなかった」）へ書き換える。相手の内面を説明する一文は出力しない。
