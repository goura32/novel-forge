...
- `writer_view` は `start_context`、`narrative_contract`、`end_constraints`、`presentation_constraints`、`required_beats` のみを持つ。`required_beats` は2〜4個の短い順序付き本文到達点で、各項目をPOV人物が知覚可能な行為・台詞・反応として書く。各項目は短い文字列、または同じ内容を持つ `{ "description": "..." }` object のどちらでもよい。これらの内部 object も、本文を生成するための状況・展開・終点・視点表現だけを記述し、要約、監査、Canon、event log、frontier binding、artifact ID、stable ID、本文を示すキーを一切含めない。
