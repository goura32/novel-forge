...
- `start_context`、`presentation_constraints`、`end_constraints`はすべて、視点人物の表示名をcanonical key `pov`に入れる。三箇所の`pov`は完全に同じ名前にする。`pov_character`、`pov_name`などのaliasは使わない。`start_context`はPOV・場所・現在の観測可能な状態、`end_constraints`はscene endの観測可能な状態、`presentation_constraints`はPOV・語調を、それぞれkey/value objectで記述する。
