...
class DesignMixin(NovelEngineBase):  # type: ignore[misc]
    ...
    def design(self, volume_number: int | None = None) -> dict[str, Any]:
        ...
        self._log.info(
            f"  ✓ chapter_design — vol={vol_num} {len(chapters)}/{chapters_count} ch done"
        )
        ...
        for ch in result.get("chapters", []):
            ...
            ch_path = (
                vol_dir
                / f"vol{vol_num:02d}_ch{ch_num:02d}"
                / f"vol{vol_num:02d}_ch{ch_num:02d}.json"
            )
        ...
        for sc in result.get("scenes", []):
            ...
            sc_path = (
                vol_dir
                / f"vol{vol_num:02d}_ch{ch_num:02d}"
                / f"vol{vol_num:02d}_ch{ch_num:02d}_sc{sc_num:02d}"
        ...
        self._save()
        self._log.info(
            f"✓ Design: series='{slug}' vol={vol_num} — {len(chapters)} ch, {len(scenes)} sc"
        )
        return result

    # ── Phase 1: Volume design ──────────────────────────────────────────

    def _generate_volume_design(
        self, series_plan: str, genre: str, vol_num: int, system: str
    ) -> list[dict]:
    ...
    def _generate_chapter_designs(
        self, chapters: list[dict], series_plan: str, vol_num: int, system: str
    ) -> list[dict]:
        designs = []
        total = len(chapters)
        for i, ch in enumerate(chapters, 1):
            ...
            design = self._llm.complete_json(
                "chapter_design", system, prompt, get_schema("chapter_design")
            )
            design["number"] = i
            ...
            for sc_idx in range(scene_count):
                prompt = self._prompts.render(
                    "scene_design.md",
                    {
                        ...
                        "chapter_title": ch_title,
                        "chapter_purpose": ch_purpose,
                        "scene_number": str(counter),
                        "chapter_scene_number": str(sc_idx + 1),
                        "chapter_scene_count": str(scene_count),
                        "lang": self._lang,
                    },
                )
                scene = self._llm.complete_json(
                    "scene_design", system, prompt, get_schema("scene_design")
                )
